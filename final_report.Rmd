...
Firstly, I classify each injury by body part; Secondly, I standardize dates and summarize all the date into 10 seasons; Thirdly, paste season and players name to a new variable join_c to prepare for joining datasets.

To wrangle datasets for further analysis:
After I cleaned two datasets, I left join them together by the variable: join_c and select the variables that I need for further analysis. 

Here are some explanations of variables:
...
ggplot(df_rs_aver_final, aes(x = Season, y = X3PAPG, fill = Season)) +
  ...
  ggtitle("Number of 3 Points attempt per game distribution from season 2010-2011 to season 2019-2020")
```
